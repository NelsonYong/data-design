...
export function nearestPoint({ dyadicArray, targetPoint, step = 1000, keep = -1 }: NearestPointType): any {
      ...
      dyadicArray?.filter((item) =>
        item instanceof Array
          ? item[0] <= targetX + keep &&
            ...
            item[1] <= targetY + keep &&
            item[1] >= targetY - keep
          : item.x <= targetX + keep && item.x >= targetX - keep && item.y <= targetY + keep && item.y >= targetY - keep
      ) ?? [];
